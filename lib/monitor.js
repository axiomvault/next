...
async function checkTransactionStatus(network, address, expectedAmount) {
  ...
  return await tryEvmProvider(rpcProviders[network], async (provider) => {
    ...
    const fromBlock = Math.max(latestBlock - 1500, 0); // ~50 min on BSC

    console.log(`📡 Searching USDT transfers from block ${fromBlock} to ${latestBlock}`);

    // Fetch ALL transfers in range, filter in JS
    ...
    console.log(`📦 Found ${logs.length} USDT transfer logs in block range`);
    ...
    for (const log of logs) {
      ...
      console.log(`🔍 Log: to=${toAddress}, from=${fromAddress}, amount=${amount}, tx=${log.transactionHash}`);
    ...
    console.log(`⏳ No matching transfer found for ${address}`);
