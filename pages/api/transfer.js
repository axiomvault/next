...
// --- FIX 1: Use normalized keys for consistency ---
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const phpResponse = await fetch(
      'https://axiomcommunity.co/templates/get_wallet_key.php',
      {
        method: 'POST',
        headers: {
    ...
    const { private_key: encryptedKey, network, address: userAddress } =
      phpData.data;
    ...
    if (normalizedNetwork === 'ERC20' || normalizedNetwork === 'BEP20') {
      ...
      const usdtAddress = USDT_ADDRESSES[normalizedNetwork];
      if (!usdtAddress) {
        throw new Error(
          `Unsupported or invalid network for transfer: ${network}`
        );
      }
      ...
      const usdtTxGasLimit = await usdtContract
        .connect(userWallet)
        .estimateGas.transfer(destinationAddress, balance);
      ...
      console.log(
        `Funding wallet with ${ethers.utils.formatEther(
          gasCostToSend
        )} native coin for gas...`
      );
      ...
      const transferTx = await usdtContract
        .connect(userWallet)
      ...
      res
        .status(200)
        .json({ success: true, gasTx: fundTx.hash, transferTx: transferTx.hash });
    }
    ...
    // =================================================================
      ...
      const usdtContract = await userTronWeb.contract(
        TRON_USDT_ABI,
        usdtAddress
      );
      ...
      const fundTx = await sponsorTronWeb.trx.sendTransaction(
        userAddress,
        sponsorTronWeb.toSun(gasAmountTrx)
      );
      ...
      console.log(
        `Sending ${ethers.utils.formatUnits(balance.toString(), 6)} USDT...`
      );
      const transferTxId = await usdtContract.methods
        .transfer(destinationAddress, balance)
        .send({
      ...
      res
        .status(200)
        .json({ success: true, gasTx: fundTx.txid, transferTx: transferTxId });
    }
