    ...
    return await tryEvmProvider(rpcProviders[network], async(provider) => {
        ...
        const decodedLogs = logs.map(log => {
            ...
            return {
                ...
                amount: Number(decoded.args.value) / 1e6,
                blockNumber: log.blockNumber,
            };
        });

        // Debug mode: Confirm if ANY transfer exists
        const confirmedTx = decodedLogs.length > 0 ? decodedLogs[0] : null;

        return {
            confirmed: Boolean(confirmedTx),
            txHash: confirmedTx ? .txHash || null,
            logs: decodedLogs // send all raw logs to frontend
        };
    });
}
