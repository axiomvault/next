...
// Using ethers.js v6. The import and usage are different from v5.
...
/**
 * Checks for a confirmed USDT transaction to a specific address using ethers v6.
 *
...
async function checkTransactionStatus(network, recipientAddress, expectedAmount) {
    ...
    try {
        ...
        if (lowerNetwork === 'bep20') lowerNetwork = 'bsc';
        if (lowerNetwork === 'erc20') lowerNetwork = 'eth';
        ...
        // 2. Connect to a Blockchain Node using ethers v6 syntax.
        // StaticJsonRpcProvider is removed in v6; use JsonRpcProvider instead.
        const provider = new ethers.JsonRpcProvider(rpcUrls[0]);
        
        // 3. Setup Contract Instance using ethers v6 syntax.
        ...
        for (const event of events.reverse()) {
            ...
            // Use ethers.utils.formatUnits with ethers v6 syntax.
            // Note: v6 uses BigInt natively, but formatUnits handles the conversion.
            const amountReceived = parseFloat(ethers.formatUnits(event.args.value, 6)); 
