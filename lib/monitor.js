...
async function checkTransactionStatus(network, address, expectedAmount) {
    ...
    return await tryEvmProvider(rpcProviders[network], async(provider) => {
        ...
        for (const log of logs) {
            ...
            const amount = parseFloat(ethers.formatUnits(decoded.args.value, 6)); // ✅ USDT 6 decimals
        ...
        // ✅ No match found
        return { status: "pending", confirmed: false, debug: debugLogs };
    });
}
